...
int CControllerConverter::convert(daeDatabase* pDatabase)
{
  ...
  for(unsigned int i = 0; i < numElements; i++)
  {
    ...
    if(pOgreMesh.isNull())
    {
      std::cerr << "Mesh " << skinRef->getSource().getElement()->getID() << " not found for skinning, skipping..." << std::endl;
      return 1;
    }
...
void CControllerConverter::transformVertices(Ogre::MeshPtr pOgreMesh, Ogre::Matrix4 *pMatTransform)
{
  ...
  {
    SubMesh* submesh = pOgreMesh->getSubMesh(i);
    ...
    if(submesh->useSharedVertices)
    {
      if(!added_shared)
      {
        VertexData* vertex_data = pOgreMesh->sharedVertexData;
